...
def render_traversal_report(manifest: dict[str, Any], ledger: dict[str, Any]) -> str:
    """Render a deterministic human-readable ledger from canonical machine evidence."""
    entries = ledger.get("entries", [])
    by_id = {entry.get("chunk_id"): entry for entry in entries if isinstance(entry, dict)}
    chunks = manifest.get("chunks", [])
    source = manifest.get("source", {})
    lines = [
        "# Traversal Report",
        "",
        f"- Source: `{_md_escape(str(source.get('uri') or source.get('path') or 'unknown'))}`",
        f"- Source SHA-256: `{source.get('sha256', '')}`",
        f"- Emitted chunks: {len(chunks)}",
        "",
        "| Chunk ID | Locator | Outcome | Verified assertion / reason |",
        "|---|---|---|---|",
    ]
    for chunk in chunks:
        chunk_id = str(chunk.get("id", ""))
        locator = json.dumps(chunk.get("locator", {}), ensure_ascii=False, sort_keys=True, separators=(",", ":"))
        entry = by_id.get(chunk_id, {})
        outcome = str(entry.get("outcome", "missing"))
        if outcome == "evidence":
            atoms = []
            for kind in ("assertions", "constraints"):
                label = "assertion" if kind == "assertions" else "constraint"
                for atom in entry.get(kind, []) if isinstance(entry.get(kind, []), list) else []:
                    if isinstance(atom, dict):
                        atoms.append(f"[{label}] {atom.get('text', '')}")
            if not atoms:
                atoms = ["[missing extractive evidence]"]
            for detail in atoms:
                lines.append(
                    f"| `{_md_escape(chunk_id)}` | `{_md_escape(locator)}` | `evidence` | {_md_escape(detail)} |"
                )
        elif outcome == "non_match":
            reason = str(entry.get("reason", ""))
            lines.append(
                f"| `{_md_escape(chunk_id)}` | `{_md_escape(locator)}` | `non_match` | {_md_escape(reason)} |"
            )
        else:
            lines.append(
                f"| `{_md_escape(chunk_id)}` | `{_md_escape(locator)}` | `{_md_escape(outcome)}` | [missing verdict] |"
            )
    return "\n".join(lines) + "\n"
